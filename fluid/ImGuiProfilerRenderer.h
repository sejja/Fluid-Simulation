#ifndef _IM_GUI_PROFILER_RENDERER__H_
#define _IM_GUI_PROFILER_RENDERER__H_
...
{
    glm::vec2 Vec2(const ImVec2& vec);
  class ProfilerGraph
  {
  public:
    int frameWidth;
  ...
  public:
    ...
    void Render();
    void SerializeFrameInfo(std::pair<std::vector<double>, std::vector<std::string>>& frame);
    void LoadFrameInfo(std::pair<std::vector<double>, std::vector<std::string>>& frame);

    bool stopProfiling;
    int frameOffset;
    ProfilerGraph cpuGraph;
    std::pair<std::vector<double>, std::vector<std::string>> capturedFrames[2];
    bool usingcapturedframes[2] = { false, false };
    int frameWidth;
    int frameSpacing;
    bool useColoredLegendText;
